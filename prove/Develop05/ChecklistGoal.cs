...
class ChecklistGoal : Goal
{
    ...
    public override UiMenu MakeEditMenu()
    {
        ...
        editMenu.AddOptionFromEnd(new UiOption(new Action(()=>{_bonusValue = Inp.GetIntInputForbid($"What would you like to change the completion bonus value to? (Currently: {_bonusValue}): ", 0); throw new OperationCanceledException();}),"B&onus Completion Value"),0);
        editMenu.AddOptionFromEnd(new UiOption(new Action(()=>{_bonusCompGoal = Inp.GetIntInputMin($"What would you like to change the bonus completion count goal to? (Currently: {_bonusCompGoal}): ",1); throw new OperationCanceledException();}),"Bonus Completion Count &Goal"),0);
